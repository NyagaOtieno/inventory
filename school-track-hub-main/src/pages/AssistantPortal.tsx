...
// API endpoints
const STUDENTS_API = 'https://schooltransport-production.up.railway.app/api/students';
const MANIFEST_API = 'https://schooltransport-production.up.railway.app/api/manifests';
const GPS_API = 'https://myfleet.track-loc8.com/api/v1/unit.json?key=44e824d4f70647af1bb9a314b4de7e73951c8ad6';

export default function AssistantPortal() {
  // TODO: replace with dynamic assistantId from JWT
  const assistantId = 165;
  ...
  const checkMutation = useMutation({
    mutationFn: async ({ studentId, status }: { studentId: number; status: 'CHECKED_IN' | 'CHECKED_OUT' }) => {
      const unit = gpsData?.find((u: any) => u.number === bus?.plateNumber);
      const latitude = unit?.lat || 0;
      const longitude = unit?.lng || 0;

      const body = { studentId, busId: bus?.id, assistantId, status, latitude, longitude };
      const res = await axios.post(MANIFEST_API, body);
      return res.data;
    },
    onSuccess: (data, variables) => {
      toast.success(`Manifest updated: Student ID ${variables.studentId} is now ${variables.status === 'CHECKED_IN' ? 'Active' : 'Inactive'}`);
  ...
  if (studentsLoading) return <p>Loading assistant info...</p>;
  if (studentsError) return <p className="text-red-500">Error loading students.</p>;
  if (!bus) return <p>Could not load bus for this assistant.</p>;
  ...
  return (
    <div className="min-h-screen bg-muted/30 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Bus + Map */}
        <Card>
          <CardHeader><CardTitle>My Bus</CardTitle></CardHeader>
          <CardContent>
              ...
              {busLocation ? (
                ...
                >
                  ...
                  <Marker position={[busLocation.lat, busLocation.lng]}>
                    <Popup>{bus.name} ({bus.plateNumber})</Popup>
                  </Marker>
                </MapContainer>
              ) : (
                ...
                <div key={student.id} className="flex items-center justify-between p-4 bg-muted rounded-lg">
                  ...
                  <div className="flex items-center gap-3">
                    {isActive ? (
                      ...
                      <Badge className="bg-muted-foreground">Inactive</Badge>
                    ) : (
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        onClick={() => checkMutation.mutate({ studentId: student.id, status: 'CHECKED_IN' })}
                      >
                        Check In
                      </Button>
                      <Button
                        ...
                        onClick={() => checkMutation.mutate({ studentId: student.id, status: 'CHECKED_OUT' })}
                      >
                        Check Out
                      </Button>
                    </div>
